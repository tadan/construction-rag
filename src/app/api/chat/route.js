...
import { generateMockResponse, shouldUseMockResponse } from "@/app/api/utils/mockAI";
...
export async function POST(request) {
  try {
    ...
    // Try Gemini API first, fallback to mock on error
    let responseStream;
    let usedMock = false;

    try {
      const geminiApiKey = process.env.GEMINI_API_KEY;
      if (!geminiApiKey || shouldUseMockResponse()) {
        throw new Error("Using mock responses for demo");
      }

      // Call Gemini API with streaming
      responseStream = await streamGeminiResponse(messages, geminiApiKey);
    } catch (error) {
      console.log("Gemini API unavailable, using mock response:", error.message);
      // Use mock response as fallback
      responseStream = generateMockResponse(message);
      usedMock = true;
    }
